...
import {
    ...
    Phone, ShieldCheck, HeartPulse, Store, Sun, Cloud, Moon
} from "lucide-react"
...
const groupSlotsByShift = (slots: any[]): Map<ShiftType, any[]> => {
    const grouped = new Map<ShiftType, any[]>()
...
const errorMessageMap: Record<string, string> = {
    ...
    APP_MESSAGE_4006: "Khung giờ không hợp lệ hoặc không thuộc phòng khám đã chọn",
    APP_MESSAGE_4007: "Khung giờ này đã hết chỗ hoặc không có bác sĩ rảnh, vui lòng chọn khung giờ khác",
...
export default function BookByClinicPage() {
    ...
    useEffect(() => {
        ...
        const loadInitialData = async () => {
            try {
                const [clinicRes, serviceRes, profileRes] = await Promise.all([
    ...
    const loadSlots = useCallback(async () => {
        ...
        try {
            ...
            const res = await patientAppointmentService.getClinicAvailableSlots(
                clinicId,
                dateStr,
                serviceId || undefined
            )
    ...
    }, [clinicId, selectedDate, serviceId])
    ...
    }, [clinicId, selectedDate, loadSlots])

    const isSlotInPast = (slot: any): boolean => {
    ...
    const hasAvailableSlots = (slots: any[]): boolean => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!patientId || !clinicId || !selectedSlot) {
            setError(isVI ? "Vui lòng chọn đầy đủ Hồ sơ bệnh nhân và Khung giờ khám" : "Please select Patient Profile and Timeslot")
            return
        }

        setSubmitting(true)
        try {
            const res = await patientAppointmentService.bookAppointmentByClinic({
                clinicId,
                patientId,
    ...
    return (
        <div className="min-h-screen bg-[#F0F4FF] text-slate-800 font-sans flex flex-col">
            ...
            <main className="flex-1 max-w-7xl w-full mx-auto px-4 md:px-6 py-8 flex flex-col items-center justify-center">
                <div className="w-full mb-6 align-self-start">
                    <button
                        onClick={() => router.back()}
                        className="inline-flex items-center gap-1.5 text-slate-500 hover:text-primary text-xs font-semibold transition"
                    >
                ...
                {!clinicId ? (
                    <div className="max-w-md w-full mx-auto py-16 bg-white border border-slate-200/80 rounded-2xl shadow-sm text-center space-y-4 px-6">
                    ...
                    <div className="w-full flex flex-col lg:flex-row gap-6 items-start">
                        <aside className="w-full lg:w-[340px] shrink-0 flex flex-col gap-4 lg:sticky lg:top-24">
                            ...
                            <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-sm">
                                ...
                                <div className="space-y-1">
                                    <Row label={isVI ? "Phòng khám" : "Clinic"} value={clinic?.name} />
                                    <Row label={isVI ? "Dịch vụ" : "Service"} value={selectedService ? `${selectedService.serviceName} (${selectedService.price ? formatCurrency(selectedService.price) : "0đ"})` : (isVI ? "Chưa chọn" : "Not selected")} />
                            ...
                            <div className="bg-gradient-to-br from-emerald-500 via-green-600 to-teal-700 rounded-2xl p-5 text-white shadow-md relative overflow-hidden">
                                ...
                                <div className="relative z-10">
                                    ...
                                    <h5 className="font-bold text-sm leading-snug mb-1">
                                        {isVI ? "Đặt lịch nhanh chóng" : "Quick Booking"}
                                    </h5>
                                    <p className="text-[11px] text-white/80 leading-relaxed">
                                        {isVI
                                            ? "Không cần chọn bác sĩ cụ thể. Hệ thống sẽ tự động sắp xếp bác sĩ phù hợp."
                                            : "No need to pick a specific doctor. We'll assign the best available doctor."}
                                    </p>
                                </div>
                            </div>
                        </aside>
                        ...
                        <div className="flex-1 w-full bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm">
                            <h2 className="text-lg font-bold text-slate-800 mb-6 pb-3 border-b border-slate-100">
                                {isVI ? "Đặt lịch theo khung giờ phòng khám" : "Book by Clinic Schedule"}
                            </h2>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-5">
                                ...
                                <div className="space-y-2">
                                    <label className="text-xs font-bold text-slate-700 flex items-center gap-1.5">
                                        ...
                                        {isVI ? "Dịch vụ/Gói khám" : "Medical Service"}
                                    </label>
                                    {loadingServices ? (
                                        <div className="flex items-center gap-2 text-xs text-slate-400"><Loader2 className="w-3.5 h-3.5 animate-spin" /></div>
                                    ) : (
                                ...
                                <div className="space-y-2 pt-1">
                                    <label className="text-xs font-bold text-slate-700 flex items-center gap-1.5">
                                        ...
                                        {isVI ? "Chọn Ngày Khám" : "Select Appointment Date"} <span className="text-rose-500">*</span>
                                    </label>
                                ...
                                {selectedDate && (
                                    <div className="space-y-3 pt-2">
                                        <label className="text-xs font-bold text-slate-700 flex items-center gap-1.5">
                                            ...
                                            {isVI ? "Chọn Khung Giờ Khám" : "Select Timeslot"} <span className="text-rose-500">*</span>
                                        </label>
                                        ...
                                        {loadingSlots ? (
                                            ...
                                            <p className="text-xs text-amber-600 bg-amber-50 border border-amber-100 rounded-xl p-3">
                                                {isVI ? "Không có khung giờ khả dụng trong ngày này." : "No available timeslots for this day."}
                                            </p>
                                        ) : (
                                            <div className="space-y-4 max-h-[350px] overflow-y-auto pr-1">
                                                {Array.from(groupSlotsByShift(slots).entries()).map(([shift, shiftSlots]) => {
                                                    const validSlots = shiftSlots.filter(slot => !isSlotInPast(slot))
                                                    if (validSlots.length === 0) return null

                                                    const config = SHIFT_CONFIG[shift]
                                                    const hasAvailable = validSlots.some(slot => slot.isAvailable)
                                                        ...
                                                        <div key={shift} className="space-y-2">
                                                            ...
                                                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                                                {shiftSlots.map((slot) => {
                                                                    ...
                                                                    const noDoctor = slot.isAvailable === false && !past
                                                                    ...
                                                                    return (
                                                                        <button
                                                                            ...
                                                                            className={`py-2.5 px-3 border text-xs font-bold rounded-xl transition-all flex flex-col items-center justify-center relative ${selected
                                                                        ...
                                                                        >
                                                                            ...
                                                                            <span className={`text-[10px] font-medium mt-0.5 ${selected ? "text-blue-100" :
                                                                                past ? "text-slate-400" :
                                                                                    noDoctor ? "text-amber-600" :
                                                                                        "text-slate-400"
                                                                                }`}>
                                                                                {past ? (isVI ? 'Đã qua' : 'Passed') :
                                                                                    noDoctor ? (isVI ? 'Không có bác sĩ' : 'No doctor') :
                                                                                        (isVI ? 'Có bác sĩ' : 'Available')}
                                                                            </span>
                                                                        </button>
                                                                    )
                                                                })}
